...
class Cfg2Station(KaitaiStruct):
    def __init__(self, _io, _parent=None, _root=None):
        ...
        for i in range(self.phnmr):
            self.phunit[i] = self._root.Phunit(self._io, self, self._root)

        self.anunit = [None] * (self.annmr)
        for i in range(self.annmr):
            self.anunit[i] = self._root.Anunit(self._io, self, self._root)
    ...
    class Phunit(KaitaiStruct):
        ...
        def __init__(self, _io, _parent=None, _root=None):
            ...
            self._parent = _parent
            self._root = _root if _root else self
            self.voltage_or_current = self._root.Phunit.VoltageOrCurrentEnum(self._io.read_u1())
