...
class Pcap(KaitaiStruct):
    ...
    class Header(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.raw_network = self._io.read_u4le()
            try:
                self.network = self._root.Linktype(self.raw_network)
            except:
    ...
    class Packet(KaitaiStruct):
        ...
        @property
        def time_epoch(self):
            if hasattr(self, '_m_time_epoch'):
                return self._m_time_epoch if hasattr(self, '_m_time_epoch') else None
    
            self._m_time_epoch = self.ts_sec + self.ts_usec / 1e6
            return self._m_time_epoch if hasattr(self, '_m_time_epoch') else None
